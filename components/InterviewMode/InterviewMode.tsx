...
const extractPromptFromResponse = (response: string): string => {
  ...
  const assignmentIndex = response.indexOf("Question: ");
  if (assignmentIndex === -1) {
    return "";
  }
  // returns the string after 'Assignment: '
  return response.slice(assignmentIndex + "Question: ".length);
};

const InterviewMode = (props: Props) => {

  const { messages, setMessages, tags, jobDescription, companyDescription, experience } = props;
  ...
  const handleSendText = async () => {
    ...
    const prevquestion = extractPromptFromResponse(previousGPT3Response);

    const prompt = `You're an interviewer currently interviewing a candidate for a job.\n\nJob Description: ${jobDescription}\n\nCompany Description: ${companyDescription}\n\nExperience: ${experience}\n\nCandidateSkills: ${tags.join(",")}\n\nPrevious Question: ${prevquestion}\n\nAnswer: ${input}\n\nGive me feedback on my answer, mistakes I made and areas I need to improve on and ask me the next question.\n\n 
    ...
    <div className="relative h-screen flex flex-col">
      <div className="flex-1 overflow-y-auto p-4 pb-16">
        {messages.map((message) => (
          ...
          >
            {message.isLoading ? <Loader /> : <ReactMarkdown>{preprocessMarkdown(message.text)}</ReactMarkdown>}
          </div>
        ))}
